...
# 通过读取文本文件分析
def word_top(str):
    article = open('data/' + 'txt/' + str + '.txt', 'r', encoding='utf-8').read()
    dele = {'。', '！', '？', '的', '“', '”', '（', '）', ' ', '》', '《', '，', '你们', '自己', '我们', '他们'}
...
def analysis(name):
    # 读取文件
    fn = open('data/txt/a.txt', 'rt', encoding='utf-8')  # 打开文件
    ...
    # 分词并去除停用词
    remove_words = set()
    fr = open('data/stopword.txt', encoding='UTF-8')
    for word in fr:
